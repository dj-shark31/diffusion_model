...
class VAESampler:
    ...
    def _extract_model_config(self, model_state: dict) -> dict:
        ...
        # Extract hidden dimensions from encoder
        hidden_dims = []
        for key in sorted(model_state.keys()):
            if 'encoder.encoder' in key and '.conv' in key and '.weight' in key:
                # Extract layer number and output channels
                parts = key.split('.')
                for i, part in enumerate(parts):
                    if part == 'conv' and i + 1 < len(parts) and parts[i + 1].isdigit():
                        conv_num = int(parts[i + 1])
                        if conv_num % 3 == 1:  # Every 3rd conv layer (Conv2d, GroupNorm, SiLU)
                            out_channels = model_state[key].shape[0]
                            hidden_dims.append(out_channels)
                            break

        # Reverse decoder hidden dims to get encoder hidden dims
        if hidden_dims:
            # Remove the last layer (output layer) and reverse
        ...
        for key in model_state.keys():
            if 'encoder.encoder.0.weight' in key:  # First conv layer
                ...
                config['image_size'] = model_state[key].shape[0]
                break
